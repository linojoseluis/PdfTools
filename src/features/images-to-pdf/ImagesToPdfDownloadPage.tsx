...
type ImagesToPdfDownloadState = {
  pdfBytes: Uint8Array;
  filename: string;
};

export function ImagesToPdfDownloadPage() {
  const location = useLocation();
  const navigate = useNavigate();
  const { downloadPdf } = useDownload();
  const state = location.state as ImagesToPdfDownloadState | null;

  useEffect(() => {
    if (!state?.pdfBytes) {
      navigate('/images-to-pdf', { replace: true });
    }
  }, [state, navigate]);
